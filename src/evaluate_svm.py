...
def evaluate():
    ...
    # Accuracy
	test_predict_svm = model.predict(testset_embedding)
	
	accuracy_score_test_svm = accuracy_score(test_predict_svm, testset_df['polarity'])
	print(accuracy_score_test_svm)

	# save the metric file
	with open(metrics_file, 'w') as fd:
	    fd.write('{:4f}\n'.format(accuracy_score_test_svm))
